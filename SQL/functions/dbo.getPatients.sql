CREATE FUNCTION dbo.getPatients(@PatientNif VARCHAR(9)=NULL,@PatientName VARCHAR(30)=NULL,@Sex varchar(1)=NULL, @authorization BIT=NULL,@RoomNumber INT=NULL,@PhoneNUmber INT=NULL,@Checkout BIT=NULL,@PageNumber INT, @RowsPage INT)
RETURNS TABLE
AS
	RETURN(
		...
		WHERE    (@PatientNif IS NULL OR  exemplo1.PATIENT.NIF = @PatientNif)
				...
				AND
					(@Checkout IS NOT NULL OR exemplo1.Patient.Check_out IS NULL ) ORDER BY exemplo1.PATIENT.NIF
