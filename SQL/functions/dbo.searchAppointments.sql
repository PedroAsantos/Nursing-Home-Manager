CREATE FUNCTION dbo.searchAppointments(@DoctorNif VARCHAR(9)=NULL,@PatientNif VARCHAR(9)=NULL,@DoctorName varchar(30)=NULL,@PatientName varchar(30)=NULL,@Date DateTime=NULL,@Speciality varchar(30)=NULL, @PageNumber INT, @RowsPage INT)
RETURNS TABLE
AS
	RETURN
		...
		WHERE    (@DoctorNif IS NULL OR exemplo1.APPOINTMENT.E_DoctorNIF = @DoctorNif)
				...
				AND
					(@Speciality IS NULL OR exemplo1.APPOINTMENT.Speciality = @Speciality)  ORDER BY exemplo1.PATIENT.NIF
